...
@testset "Comprehensive ITensors Operator Mapping" begin
    ...
    @testset "Single-Qubit & Aliases" begin
        ...
        # Parametric
        @test to_itensor_op(ParametricSingleGate(1, :Rx, [0.1]), sites) ≈
            op("Rx", sites[1]; θ=0.1)
        @test to_itensor_op(ParametricSingleGate(1, :Rn, [0.1, 0.2, 0.3]), sites) ≈
            op("Rn", sites[1]; θ=0.1, ϕ=0.2, λ=0.3)
    end
    ...
    @testset "Two-Qubit Gates" begin
        # Standard
        @test to_itensor_op(ControlledGate(1, 2, :CNOT), sites) ≈
            op("CNOT", sites[1], sites[2])
        @test to_itensor_op(ControlledGate(1, 2, :CZ), sites) ≈ op("CZ", sites[1], sites[2])
        @test to_itensor_op(TwoQubitGate(1, 2, :SWAP), sites) ≈
            op("SWAP", sites[1], sites[2])
        @test to_itensor_op(TwoQubitGate(1, 2, :iSWAP), sites) ≈
            op("iSWAP", sites[1], sites[2])

        # Parametric Controlled
        @test to_itensor_op(ParametricControlledGate(1, 2, :CRx, [0.5]), sites) ≈
            op("CRx", sites[1], sites[2]; θ=0.5)

        # Ising Coupling
        @test to_itensor_op(ParametricTwoQubitGate(1, 2, :Rxx, [0.4]), sites) ≈
            op("Rxx", sites[1], sites[2]; ϕ=0.4)
    end

    @testset "Multi-Qubit Gates" begin
        @test to_itensor_op(ThreeQubitGate(1, 2, 3, :Toffoli), sites) ≈
            op("Toffoli", sites[1], sites[2], sites[3])
        @test to_itensor_op(ThreeQubitGate(1, 2, 3, :Fredkin), sites) ≈
            op("Fredkin", sites[1], sites[2], sites[3])
        @test to_itensor_op(FourQubitGate(1, 2, 3, 4, :CCCNOT), sites) ≈
            op("CCCNOT", sites[1], sites[2], sites[3], sites[4])
    end
end
